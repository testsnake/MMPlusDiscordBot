...
async function checkGamebananaFeed() {
    ...
    if (feedEnabled) {
        ...
        if (latestTimestamp === undefined || latestTimestamp === null || typeof latestTimestamp != 'number' || isNaN(latestTimestamp)) {
            log.info(`latestTimestamp is ${latestTimestamp}, setting to 1 hour ago`);
...
async function showRecord(subType, submissionId, isNew) {
    ...
    try {
        ...
        modInfo = await fetch(`https://gamebanana.com/apiv10/${subType}/${submissionId}/ProfilePage`).then(res => {
            if (res.status !== 200) {
